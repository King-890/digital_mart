...
router.post('/order', async (req, res) => {
    try {
        ...
        const mailOptions = {
            from: `"Gaya ji Digital Mart Notifications" <${process.env.ADMIN_EMAIL}>`,
            ...
            html: `
                <div style="font-family: sans-serif; max-width: 600px; border: 1px solid ${isFraud ? '#ff4444' : '#eee'}; padding: 20px;">
                    ...
                    <p style="margin-top: 20px; font-size: 0.8rem; color: #888;">
                        This is an automated notification from Gaya ji Digital Mart System.
                    </p>
                </div>
            `
        };
        ...
        if (order.shippingAddress && order.shippingAddress.includes('Phone:')) {
            ...
            if (phoneMatch && phoneMatch[1]) {
                ...
                await sendSMS(
                    userPhone,
                    `Hi! Your Gaya ji Digital Mart order #${order.id} for ₹${order.totalAmount} has been placed. Track it here: ${process.env.FRONTEND_URL || 'http://localhost:5173'}/track/${order.id}`
                );
            }
        }
...
router.post('/contact', async (req, res) => {
    try {
        ...
        const mailOptions = {
            from: `"Gaya ji Digital Mart Contact" <${process.env.ADMIN_EMAIL}>`,
            to: process.env.ADMIN_EMAIL,
...
router.post('/return', async (req, res) => {
    try {
        ...
        const mailOptions = {
            from: `"Gaya ji Digital Mart Returns" <${process.env.ADMIN_EMAIL}>`,
            to: process.env.ADMIN_EMAIL,
            ...
            html: `
                <div style="font-family: sans-serif; max-width: 600px; border: 1px solid #fca5a5; padding: 20px; border-radius: 8px;">
                    ...
                    <p style="margin-top: 20px; font-size: 0.8rem; color: #64748b; border-top: 1px solid #eee; padding-top: 12px;">
                        This is an automated notification from Gaya ji Digital Mart System. Please follow up with the customer within 24-48 hours.
                    </p>
                </div>
            `
        };
